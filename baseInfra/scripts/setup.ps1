...
if (Test-Path $SentinelFile) {
    if ((Get-StageStatus -Stage 'devpost') -ne 'success') {
        ...
        # Create startup task under SYSTEM so it runs even without an interactive logon
        $psExe = "$env:WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe"
        schtasks /Create /TN $ScheduledTaskName /TR "`"$psExe`" -NoLogo -NoProfile -ExecutionPolicy Bypass -File `"$postScript`" -StatusFile $StatusFile -LogDir $LogDir" /SC ONSTART /RU SYSTEM /RL HIGHEST /F | Out-Null
        Info "Reboot in 60 seconds to continue with devpost stage."
